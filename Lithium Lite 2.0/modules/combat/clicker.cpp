...
int randomization(int cps, int& edited_cps, int& reset_value, int& clicks, int& counter,
    ...
    if (clicks >= reset_value || clicks == 0) {
        reset_value = ::utils::random_int(5, 15);
        edited_cps = cps + ::utils::random_int(-3, 3);
    ...
    if (counter >= counter_reset_value || counter == 0) {
        counter_reset_value = ::utils::random_int(15, 25);

        counter = 0;

        // DROPS

        if (!drop && !spike && ::utils::random_int(0, 100) <= 30) {
            drop = true;
            drop_reset_value = ::utils::random_int(10, 25);
            drop_counter = 0;
        }

        // SPIKES

        if (!spike && !drop && ::utils::random_int(0, 100) <= 20) {
            spike = true;
            spike_reset_value = ::utils::random_int(10, 25);
            spike_counter = 0;
        }
    }
    ...
    if (drop && drop_counter <= drop_reset_value) {
        drop_counter++;
        return ::utils::random_int(550, 600) / edited_cps;
    }
    ...
    if (spike && spike_counter <= spike_reset_value) {
        spike_counter++;
        return ::utils::random_int(450, 500) / edited_cps;
    }
    ...
    return ::utils::random_int(450, 550) / edited_cps;
}
...
void blockhit_thread() {
    while (::info::verified) {
        while (::modules::clicker::left::blockhit && ::modules::clicker::left::enabled
            ...
            if (FindWindow(("LWJGL"), nullptr) == GetForegroundWindow()) {
                if (!::local_player::inventory_open) {
                    PostMessageW(::info::minecraft_window, WM_RBUTTONDOWN, MK_RBUTTON, MAKELPARAM(0, 0));
                    std::this_thread::sleep_for(std::chrono::milliseconds((int)::utils::random_int(50, 75)));

                    PostMessageW(::info::minecraft_window, WM_RBUTTONUP, MK_RBUTTON, MAKELPARAM(0, 0));
                    std::this_thread::sleep_for(std::chrono::milliseconds((int)::utils::random_int(50, 75)));
                }
            }
            std::this_thread::sleep_for(std::chrono::milliseconds((int)::modules::clicker::left::blockhit_delay * 5));
        }
        std::this_thread::sleep_for(std::chrono::milliseconds((int)::modules::clicker::left::blockhit_delay * 5));
    }
}
...
void rmb_lock_thread() {
    while (::info::verified) {
        ...
        if (!::modules::clicker::left::rmb_lock)
            rmb_lock_held = false;
...
void ::modules::clicker::left::jitter::thread() {
    ...
    CreateThread(nullptr, 0, (LPTHREAD_START_ROUTINE)&rmb_lock_first_thread,
        nullptr, 0, nullptr);
    while (::info::verified) {
        while (::modules::clicker::left::enabled && GetAsyncKeyState(VK_LBUTTON) & 0x8000 &&
            !rmb_lock_held) {
            if (::modules::clicker::left::x_jitter + ::modules::clicker::left::y_jitter != 0) {
                POINT P;
                GetCursorPos(&P);
                ...
                if (!::local_player::inventory_open) {
                    if (FindWindow(("LWJGL"), nullptr) == GetForegroundWindow()) {
                        ...
                        if (::modules::clicker::left::slot[::local_player::current_slot -
                            1]) {
                            INT64 random = random_float(1, 12);

                            if (::modules::clicker::left::x_jitter != 0) {
                                if (random <= 3) {
                                    for (size_t f = 0;
                                        f < ::modules::clicker::left::x_jitter; f++) {
                                        GetCursorPos(&P);
                            ...
                            if (::modules::clicker::left::y_jitter != 0) {
                                if (random <= 3) {
                                    for (size_t f = 0;
                                        f < ::modules::clicker::left::y_jitter; f++) {
                                        GetCursorPos(&P);
...
void ::modules::clicker::left::thread() {
    bool first_click = true;
    while (::info::verified) {
        if (::modules::clicker::left::enabled && GetAsyncKeyState(VK_LBUTTON) & 0x8000 &&
            !rmb_lock_held) {
            while (::modules::clicker::left::enabled && GetAsyncKeyState(VK_LBUTTON) & 0x8000) {
                ...
                if (FindWindow(("LWJGL"), nullptr) == GetForegroundWindow()) {
                    ...
                    if (ScreenToClient(window, &pos1)) {
                        ...
                        if (pos1.y > 20) {
                            ...
                            if (::modules::clicker::left::inventory) {

                                if (!::local_player::inventory_open) {
                                    if (::modules::clicker::left::slot[
                                        ::local_player::current_slot - 1]) {

                                        if (::utils::random_int(0, 100) <= 2)
                                            std::this_thread::sleep_for(std::chrono::milliseconds(
                                                randomization(::modules::clicker::left::cps,
...
void ::modules::clicker::right::thread() {
    bool first_click = true;
    while (::info::verified) {
        if (::modules::clicker::right::enabled && GetAsyncKeyState(VK_RBUTTON) & 0x8000 &&
            !rmb_lock_held) {
            while (::modules::clicker::right::enabled && GetAsyncKeyState(VK_RBUTTON) & 0x8000) {
                POINT pos1;
                GetCursorPos(&pos1);
                HWND window = WindowFromPoint(pos1);
                ...
                if (FindWindow(("LWJGL"), nullptr) == GetForegroundWindow()) {
                    if (ScreenToClient(window, &pos1)) {
                        if (pos1.y > 20) {
                            ...
                            if (::modules::clicker::right::slot[::local_player::current_slot -
                                1]) {

                                if (::utils::random_int(0, 100) <= 2)
                                    std::this_thread::sleep_for(std::chrono::milliseconds(
                                        randomization(::modules::clicker::right::cps,
